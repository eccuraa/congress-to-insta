...
function fetchPublicLaws(customDate) {
  ...
  try {
    ...
    Logger.log(
      `Found ${data.bills ? data.bills.length : 0} bills in date range`,
    );
    ...
    for (const bill of data.bills) {
      if (
        ...
        const { houseVoteID, senateVoteID } = getVoteIDs(
          congress,
          bill.type.toLowerCase(),
          bill.number,
        );
        ...
        if (!houseVoteID) {
          Logger.log(
            `⊗ Removing "${bill.title}" from posting queue - not a yea-or-nay/recorded vote (voice vote)`,
          );
          continue; // Skip to next bill
        }

        const houseVoteData = houseVoteID
          ? fetchHouseVotes(congress, sessionNumber, houseVoteID)
          : { demYeas: "N/A", repYeas: "N/A", indYeas: "N/A" };

        const senateVoteData = senateVoteID
          ? fetchSenateVotes(congress, sessionNumber, senateVoteID)
          : { demYeas: "N/A", repYeas: "N/A", indYeas: "N/A" };
        ...
        Logger.log(
          `  House Votes - Dem Yeas: ${houseVoteData.demYeas}, Rep Yeas: ${houseVoteData.repYeas}, Ind Yeas: ${houseVoteData.indYeas}`,
        );
        Logger.log(
          `  Senate Votes - Dem Yeas: ${senateVoteData.demYeas}, Rep Yeas: ${senateVoteData.repYeas}, Ind Yeas:  ${senateVoteData.indYeas}`,
        );
      }
    }
  } catch (error) {
  ...
  Logger.log(
    `Complete! Found ${results.length} law(s) that became public on ${dateStr}`,
  );
}
...
function getVoteIDs(congress, billType, billNumber) {
  try {
    ...
    if (
      !billData.bill ||
      !billData.bill.actions ||
      !billData.bill.actions.url
    ) {
      Logger.log(
        `  No actions URL for ${billType.toUpperCase()} ${billNumber}`,
      );
      return null;
    }
    ...
    if (!actionsData.actions || actionsData.actions.length === 0) {
      Logger.log(
        `  No actions found for ${billType.toUpperCase()} ${billNumber}`,
      );
      return null;
    }
    ...
    Logger.log(
      `  Found roll calls for ${billType.toUpperCase()} ${billNumber} — House: ${houseVoteID}, Senate: ${senateVoteID}`,
    );
    return { houseVoteID, senateVoteID };
  } catch (error) {
    Logger.log(
      `  Error getting roll call for ${billType.toUpperCase()} ${billNumber}: ${error.toString()}`,
    );
    return null;
  }
}
...
function fetchHouseVotes(congress, sessionNumber, houseVoteID) {
  ...
  try {
    ...
    if (
      !rollCallData.houseRollCallVote ||
      !rollCallData.houseRollCallVote.votePartyTotal
    ) {
    ...
    Logger.log(
      `  Error fetching vote data for roll call ${houseVoteID}: ${error.toString()}`,
    );
    return defaultVotes;
  }
}
...
function fetchSenateVotes(congress, sessionNumber, senateVoteID) {
  ...
  if (demYeas + repYeas + indYeas !== totalYeas) {
    Logger.log(
      `Assert failed: demYeas (${demYeas}) + repYeas (${repYeas}) + indYeas (${indYeas}) !== totalYeas (${totalYeas})`,
    );
  }
