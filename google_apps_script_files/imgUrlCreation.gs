const CONFIG = {
  ...
  cloudFunctionUrl:
    "https://us-central1-voting-plot-matplotlib.cloudfunctions.net/generate_arc_image",
};
...
function generateAllImgBBUrls() {
  ...
  for (let i = 2; i <= lastRow; i++) {
    ...
    const visualSummary = postsSheet
      .getRange(i, 2)
      .getValue()
      .toString()
      .trim();
...
function exportSlideAsImage(presentationId, slideIndex) {
  ...
  if (response.getResponseCode() !== 200) {
    throw new Error(
      "Failed to export slide. Response: " + response.getResponseCode(),
    );
  }
